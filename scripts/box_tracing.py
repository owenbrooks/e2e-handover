...
class BoxTracerNode():
    ...
    def run(self):
        ...
        # move to start state
        self.move_group.plan(self.home_joint_angles)
...
def compute_force_magnitude(force_reading, initial_force):
    return sqrt((force_reading.x - initial_force.x)**2 + (force_reading.y - initial_force.y)**2 + (force_reading.z - initial_force.z)**2)
