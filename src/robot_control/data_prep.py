...
def add_recent_transition(session_id: str, lookback_frames: int):
    ...
    for trans_index in transition_indices:
        if trans_index < len(gripper_is_open) - lookback_frames:
            recent_transitions[trans_index:trans_index+lookback_frames] = True
    
    df['recent_transition'] = pd.Series(recent_transitions)

    new_annotations_file = os.path.join(data_dir, session_id, session_id + '.csv')
